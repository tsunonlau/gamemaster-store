...
// FIXED: Replace showSuccessModal() function in script.js - Fixes payer name and duplicate buttons
...
function showSuccessModal(details) {
    ...
    // FIXED: Extract payer name from multiple sources including cardholder name
    let payerName = 'N/A';
    
    // Method 1: Try to get cardholder name from billing fields (most accurate for card payments)
    ...
    if (firstNameField && lastNameField && firstNameField.value && lastNameField.value) {
        ...
        console.log('[GameMaster] ✅ Payer name from billing fields:', payerName);
    }
    // Method 2: Try PayPal response payer object
        ...
        if (givenName || surname) {
            ...
            console.log('[GameMaster] ✅ Payer name from PayPal response:', payerName);
        }
    }
    // Method 3: Try email address as fallback
    else if (details.payer && details.payer.email_address) {
        payerName = details.payer.email_address;
        console.log('[GameMaster] ✅ Payer name from email:', payerName);
    }
    ...
    // FIXED: Completely rebuild success content to avoid duplicates
    ...
    modal.classList.add('show');
    console.log('[GameMaster] 🎉 Success modal displayed - Payer:', payerName, '- Amount:', totalAmount);
}
